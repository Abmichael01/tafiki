// components/RetailShop/Register/Stage2.tsx
...
const Stage2: React.FC = () => {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: (data) => {
      console.log(data);
      navigate("/retail-shop/register?stage=3");
    },
    onError: (error: Error) => {
  ...
  const onSubmit = (values: FormData) => {
    ...
    mutate({
      ...
      email: values.email,
    } as Partial<PartnerSignupData & { user_type: string }>);
  };
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 w-full">
        ...
        <h2 className="flex gap-1 justify-center font-satoshi text-[#6E6E6E] font-[500] text-center text-[16px]">
          ...
          <Link to="/retail-shop/login" className="text-[#15221B] underline">
            Sign in
          </Link>
        </h2>
      </form>
