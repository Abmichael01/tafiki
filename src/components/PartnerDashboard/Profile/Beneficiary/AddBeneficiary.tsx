  ...
  const verifyAccountNumber = async () => {
    ...
    try {
      const response = await axios.get(
        ...
        {
          headers: {
            Authorization: "Bearer sk_test_bacb546c151e18d44f3f4aa17a98aec6379b3c55",
      ...
      if (response.data && response.data.status) {
        ...
        setVerificationError("Failed to verify account number. Please try again.");
      }
    } catch (error) {
      console.error("Error verifying account number:", error);
      setVerificationError("An error occurred while verifying the account number.");
      form.setValue("name", "");
    } finally {
      ...
      <Form {...form}>
        <div className="space-y-[12px] font-satoshi">
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="space-y-[8px]">
                <FormLabel className="text-[14px] font-[500]">Select Bank</FormLabel>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="space-y-[8px]">
                <FormLabel className="text-[14px] font-[500]">Account number</FormLabel>
                <FormControl>
                  <input
                    {...field}
                    type="text"
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="space-y-[8px]">
                <FormLabel className="text-[14px] font-[500]">Account name</FormLabel>
                <FormControl>
          ...
          <div className="pt-[48px]">
            ...
            >
              {isVerifying ? "Verifying..." : isPending ? "Adding Beneficiary..." : "Next"}
            </Button>
          </div>
        </div>
      </Form>
